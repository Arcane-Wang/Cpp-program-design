...
struct Close2EastWarriorCompare {
    bool operator()(Warrior* a, Warrior* b) {
        return a->Get_pos() > b->Get_pos();
    }
};

struct Close2WestWarriorCompare {
    bool operator()(Warrior* a, Warrior* b) {
        return a->Get_pos() < b->Get_pos();
    }
};
// bool RedWarriorCompare(Warrior* a, Warrior* b) {
//     return a->Get_pos() > b->Get_pos();
// }

// bool BlueWarriorCompare(Warrior* a, Warrior* b) {
//     return a->Get_pos() < b->Get_pos();
// }
...
public:
    ...
    virtual void After_fight(Warrior *enemy, int win_state, int enemy_hp_before_fight)  // 0: tie, 1: win, 2: lose
    {
...
public:
    ...
    {
        warriors_to_reward = vector<Warrior *>();
    ...
    {
        if (warriors_to_reward.size() == 0)
                return;
        if (hq_name == "red")
        {   
            std::sort(warriors_to_reward.begin(), warriors_to_reward.end(), Close2EastWarriorCompare());
        }
        else
        {   
            std::sort(warriors_to_reward.begin(), warriors_to_reward.end(), Close2WestWarriorCompare());
        }
    ...
    {
        for (int idx = 0; idx < cities_to_recycle_hp.size(); idx++)
        {   
            int i = cities_to_recycle_hp[idx];
...
{
    vector<Warrior *> &red_warriors = red.Get_warriors();
    std::sort(red_warriors.begin(), red_warriors.end(), Close2WestWarriorCompare());
    vector<Warrior *> &blue_warriors = blue.Get_warriors();
    std::sort(blue_warriors.begin(), blue_warriors.end(), Close2WestWarriorCompare());
